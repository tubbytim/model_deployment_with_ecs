...
S3_DATA_BUCKET ='sh-adhoc-data'
S3_DATA_KEY = 'titanic-data/titanic-data.csv'
S3_MODEL_BUCKET = 'sh-adhoc-models'
S3_MODEL_KEY = 'titanic-classifier'
...
def pre_pipeline_preparation(*, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    data.drop(labels=config.unused_fields, axis=1, inplace=True)
...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    s3 = boto3.client('s3')
    obj = s3.get_object(Bucket=S3_DATA_BUCKET, Key=S3_DATA_KEY)
...
def save_pipeline(*, pipeline_to_persist: Pipeline) -> None:
    ...
    save_file_name = f"{S3_MODEL_KEY}/{config.pipeline_save_file}{_version}.pkl"

    # WRITE
    with tempfile.TemporaryFile() as fp:
        joblib.dump(pipeline_to_persist, fp)
        fp.seek(0)
        s3.put_object(Body=fp.read(), Bucket=S3_MODEL_BUCKET, Key=save_file_name)
